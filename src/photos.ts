      ...
      for await (const attachmentKey of sheetFile.attachments) {
        for await (const image of record[keyName(attachmentKey.trim())] || []) {
          ...
          for await (const size of Object.keys(image.thumbnails || {})) {
            const hasImage = await pathExists(
            ...
            imageUrls.push(
              `https://open-data.karuna2020.org/${url}/medium.${fileExtension}`
            );
...
`
      ...
      if (imageUrls.length) {
        ...
        const updateData: any = {
          id: record._id,
          fields: {
            "CDN images": imageUrls.join(", ")
          }
        };
        if (tab === "Distribution")
          updateData.fields["Distribution Pictures"] = [];
        if (tab === "Social Media Outreach") updateData.fields.Pictures = [];
        if (tab === "Partners") updateData.fields.Logo = [];
